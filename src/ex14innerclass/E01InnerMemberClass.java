...
class OuterMember {
	...
	class InnerMember {
		...
		/*
		외부클래스는 내부클래스 보다 넓은 지역이므로 내부클래스에서는 외부의 모든 멤버를 접근할 수 있다.
		*/
	...
	public class E01InnerMemberClass {
		...
		public static void main(String[] args) {
			/*
			외부클래스가 아닌 다른 클래스에서는 내부 멤버클래스가 보이지 않으므로 직접 인스턴스를 생성할 수 없다.
			*/
			
			// InnerMember innerClass = new InnerMember(); -> 에러발생
			
			/*
			외부클래스의 인스턴스를 먼저 생성한 후 , 내부 클래스의 인스턴스를 생성해야한다.
			*/
