...
1. 이 경우 부모타입의 참조변수는 부모로부터 상속 받은 멤버까지만 접근 가능하다.
2. 자식에서 오버라이딩한 메소드가 우선 호출된다.
3. 자식에서 새롭게 정의한 멤버에는 접근할 수 없다.

- 같은 타입의 클래스로 객체를 참조
객체 전체를 접근 할 수 있다. 즉 , 클래스의 일반적인 규칙이 적용된다.
*/
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		// parent1.childMember = 1; // 접근 불가 -> 부모타입으로 자식멤버에 접근 할 수 없다.
		parent1.parentMethod(); // 오버라이딩 했으므로 자식쪽으로 호출된다.
		
		/*
		이질화 : 자식에서 새롭게 정의한 멤버 (변수 , 메소드) 에는 부모타입의 참조변수로 접근이 불가능하다.
		만약 접근해야한다면 자식타입으로 형변환 (강제형변환 혹은 Down캐스팅) 이 후 자식의 멤버에 접근해야한다.
		*/
		...
		/*
		Java 에서 생성한 모든 클래스는 직/간접적으로 Object 클래스를 상속한다.
