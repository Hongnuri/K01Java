...
public class E06Random {
	...
	public static void main(String[] args) {
		...
		만약 seed 값이 동일하다면 항상 동일한 패턴의 난수를 생성하게 되는데
		...
		*/
		...
		for (int i = 0; i < 10; i++) {
			/*
			nextInt()의 인자값 100이 전달 되면 0 ~ 99 사이의 난수가 생성된다.
			*/
		...
		// 현재 시간을 멀리 초 단위로 반환하는 메소드
		System.out.println(System.currentTimeMillis());
		
		// random 클래스를 통해 난수를 생성할 때 , seed 를 설정하기 위한 메소드
		...
		for (int i = 0; i < 10; i++) {
			/*
			nextInt() 에 인자 값이 없는 경우에는 음수 , 양수가 혼용되어 난수가 생성 된다.
			또한 , 범위가 없기 때문에 몇 자리가 생성 될지 알 수 없다.
			*/
