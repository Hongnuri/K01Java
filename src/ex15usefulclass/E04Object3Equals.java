...
class MyClass {
	...
	toString() : 
	실제 저장 된 데이터가 반환되도록 
	Object 클래스로부터 상속 받아 오버라이딩한 메소드
	 */
	@Override
	...
	*/
	...
	public boolean equals(Object obj) {
		...
		/*
		매개변수로 전달 된 객체를 Object 로 받은 후
		MyClass 의 인스턴스인지 먼저 판단한다.
		*/
		if(obj instanceof MyClass) {
			
			// Object 형으로 전달 된 매개변수를 MyClass형으로 다운캐스팅 진행
			MyClass mc = (MyClass)obj;
			
			// 두 객체의 멤버변수의 값을 실제로 비교함
			if(mc.data == this.data) {
				return true; // 값이 같으면 true 를 반환한다.
				
			} else {
				return false; // 다르면 false 로 반환한다.
			}
		}
		
	else {
		/*
		매개변수로 전달 된 객체가 MyClass 의 인스턴스가 아니라면
		비교의 대상이 될 수 없으므로 false 를 반환한다.
		*/
		return false;
	}
}
...
class Point {
	...
	/*
	객체 1 , 2의 x , y 값을 동시에 비교하여 둘 다 동일 할 때 
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		//toString() 메소드에 오버라이딩한 내용을 출력. 출력결과는 동일함.
		...
		/*
		아래 toString() 메소드는 굳이 호출하지 않아도 print()문이 실행 될 때 
