package ex08class;
		...
		{	
			...
			/*소유주
			Human 클래스를 기반으로 하는 멤버변수로 초기화를 위해 이름 , 나이 , 에너지 세 가지의 값이 필요하다.
			...
			*/
			...
			/*
			위와 동일한 이름의 함수로 오버로딩에 의해 정의 된 추기화 메소드. 
			...
			*/
			...
			void initialize(String model , int year , String name , int age , int energy)
			{
				...
				carYear = year;
				/*
				또 다른 객체가 멤버변수로 정의 되었으므로 객체를 생성한 후 
				각각의 멤버변수를 초기화해야한다.
				*/
				owner = new Human();
				owner.name = name;
		...
		public class E03CarMain 
		{
			...
			// car1.drive();
			/*
			에러발생 : 자동차 소유주를 표현하는 Human 객체가 초기화 되지 않은 상태에서 출력을 시도하여 
			런타임 에러가 발생된다.
			*/
			System.out.println("[초기화메소드 호출 후]");
			car1.initialize(); // 고정 된 내용으로 초기화 진행 된다.
			car1.drive();
			car1.showCarInfo();
