...
class FruitSeller
{
	...
	int myMoney = 0; // 판매수익
	final int APPLE_PRICE = 1000; //사과 단가
	/*
	클래스에서 멤버상수를 선언하는 경우 반드시 선언과 동시에 초기화 해야한다.
	만약 초기화를 별도로 진행하면 에러가 발생한다. (1000부분을 제거해보면 에러가 발생한다.)
	*/
	
	//판매자가 사과를 판매하는 행위를 표현한 멤버메소드
	/*
	1. 구매자로부터 금액을 전달받는다. (매개변수)
	2. 금액에 해당하는 판매 될 사과의 갯수를 구한다.
	3. 보유한 재고에서 갯수 만큼 차감한다.
	4. 판매수익이 증가한다.
...
class FruitBuyer
{
	...
	/*
	1. 판매자에게 금액을 지불한다.
	2. 판매자가 금액에 해당하는 사과의 갯수를 반환해준다. 반환한 갯수만큼 보유한 사과의 갯수가 증가한다.
	3. 판매자에게 지불한 금액 만큼 차감된다.
	 */
